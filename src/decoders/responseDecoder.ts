...
export class ResponseDecoder<T extends string> implements Decoder<T> {
  ...
  // Khởi tạo bộ giải mã phản hồi cho giao thức
  ...
  // Lấy giao thức đã được giải nén
  getUnpackedProtocol(): ProtocolSpecification<T> {
    return this.protocol;
  }

  // Xác thực tính hợp lệ của giao thức
  validateProtocol(protocol: ProtocolSpecification<T>): boolean {
    ...
    try {
      ...
      // Kiểm tra xem độ dài phản hồi có đúng hay không
  ...
  decode<T extends ResponseDataTypes = ResponseDataTypes>(
    ...
    // Lấy định nghĩa phản hồi dựa trên chữ ký
    ...
    try {
      ...
      // Duyệt qua các phần tử dữ liệu và giải mã
      for (const dataItem of responseDefinition.items) {
        ...
        // Xử lý từng loại dữ liệu khác nhau
